...
# --- Target paths (parameterized by configure()) -------------------------
# Phase 0: these module-level globals were hardcoded to the housetree `apps/gest`
# layout. configure(cfg) mutates them before any generator runs, so the ~30
# functions that read them keep their original (global-reading) shape — a
# behavior-identical port, not a 30-call-site refactor. The defaults below
# reproduce the historical housetree paths for direct/standalone use.
# TODO(phase1): thread Config explicitly instead of mutating module globals
# (the core/adapter refactor in Phase 1 removes these globals entirely).
REPO_ROOT = Path(__file__).resolve().parent.parent
GEST_ROOT = REPO_ROOT / "apps" / "gest"
SRC_MAIN_JAVA = GEST_ROOT / "src" / "main" / "java"
CONTRACTS_DIR = GEST_ROOT / "src" / "test" / "resources" / "contracts"
GENERATED_DIR = GEST_ROOT / "_generated"
APP_LABEL = DEFAULT_APP_SUBDIR


def configure(cfg: Config) -> None:
    """Point all module-level path globals at a configured target.

    Called once by the CLI before running any collect_*/render_* function. This is
    the seam that de-hardcodes `apps/gest`: every function below still reads the
    globals, but they now resolve to `cfg`'s target instead of housetree's.
    """
    global REPO_ROOT, GEST_ROOT, SRC_MAIN_JAVA, CONTRACTS_DIR, GENERATED_DIR
    global APP_LABEL, GEST_PACKAGE_ROOT, JACOCO_CSV, ADR_DIR, TODO_SCAN_ROOTS
    REPO_ROOT = cfg.repo_root
    GEST_ROOT = cfg.app_root
    SRC_MAIN_JAVA = cfg.src_main_java
    CONTRACTS_DIR = cfg.contracts_dir
    GENERATED_DIR = cfg.generated_dir
    APP_LABEL = cfg.label
    GEST_PACKAGE_ROOT = cfg.package_root
    JACOCO_CSV = GEST_ROOT / "target" / "site" / "jacoco" / "jacoco.csv"
    ADR_DIR = GEST_ROOT / "decisions"
    TODO_SCAN_ROOTS = [
        GEST_ROOT / "src",
        GEST_ROOT / "e2e" / "tests",
        GEST_ROOT / "frontend" / "src",
    ]
...
def collect_route_constants() -> dict[str, str]:
    """
    ...
    """
    out: dict[str, str] = {}
    for f in SRC_MAIN_JAVA.rglob("*Routes.java"):
...
def load_contracts() -> list[tuple[str, str, str, str]]:
    """
    ...
    if not CONTRACTS_DIR.is_dir():
        return out
    for f in sorted(CONTRACTS_DIR.rglob("*.groovy")):
...
def collect_endpoints(route_constants: dict[str, str], contracts: list[tuple[str, str, str, str]]) -> list[Endpoint]:
    endpoints: list[Endpoint] = []
    for jf in sorted(SRC_MAIN_JAVA.rglob("*.java")):
        ...
        for n in walk(root):
            ...
            for ann_name, ann in anns:
                ...
                ep = Endpoint(
                    ...
                    summary=summary,
                    file_rel=str(jf.relative_to(REPO_ROOT)),
...
def render_endpoints(endpoints: list[Endpoint]) -> str:
    lines = [f"# HTTP endpoints — {APP_LABEL} (as-is)", ""]
    lines.append(
...
def collect_events() -> list[DomainEvent]:
    ...
    for ev in out:
        ev.handlers = grep_files(rf"@EventHandler[\s\S]*?\b{ev.name}\b", SRC_MAIN_JAVA)
        ev.emitters = grep_files(rf"new\s+{ev.name}\s*\(", SRC_MAIN_JAVA)
    return out


def grep_files(pattern: str, root: Path) -> list[str]:
...
def render_events(events: list[DomainEvent]) -> str:
    lines = [f"# Domain events — {APP_LABEL} (as-is)", ""]
    lines.append(
...
def collect_projections() -> list[Projection]:
    out: list[Projection] = []
    for jf in sorted(SRC_MAIN_JAVA.rglob("*.java")):
        source, root = parse(jf)
...
def render_projections(projections: list[Projection]) -> str:
    lines = [f"# Projections — {APP_LABEL} (as-is)", ""]
    lines.append(
...
GEST_PACKAGE_ROOT = DEFAULT_PACKAGE_ROOT  # overridden by configure(); housetree default
...
def collect_modules() -> list[Module]:
    ...
    """
    java_files = sorted(SRC_MAIN_JAVA.rglob("*.java"))
    pkg_parts = tuple(p for p in GEST_PACKAGE_ROOT.split(".") if p)
    gest_root_path = SRC_MAIN_JAVA.joinpath(*pkg_parts) if pkg_parts else SRC_MAIN_JAVA
    import_re = re.compile(
        r"^import\s+(?:static\s+)?"
        + re.escape(GEST_PACKAGE_ROOT)
        + r"\.([a-z][a-zA-Z0-9_]*)\.",
        re.MULTILINE,
    )
    ...
    for pi in sorted(SRC_MAIN_JAVA.rglob("package-info.java")):
...
def render_modules(modules: list[Module]) -> str:
    lines = [f"# Modules — {APP_LABEL} (as-is)", ""]
    lines.append(
...
def parse_flyway_files() -> list[FlywayMigration]:
    mig_dir = GEST_ROOT / "src" / "main" / "resources" / "db" / "migration"
...
def render_schema(migrations: list[FlywayMigration]) -> str:
    lines = [f"# Database schema — {APP_LABEL} (as-is)", ""]
    lines.append(
...
def _collect_profile_overrides(props: list[ConfigProperty]) -> "dict[str, list[tuple[str, str, str, str]]]":
    """
    ...
    """
    res_dir = GEST_ROOT / "src" / "main" / "resources"
...
def collect_config_properties() -> list[ConfigProperty]:
    res_dir = GEST_ROOT / "src" / "main" / "resources"
    ...
    for k, cp in by_key.items():
        cp.used_by = grep_files(rf"\$\{{\s*{re.escape(k)}\b", SRC_MAIN_JAVA)
...
def render_config(items: tuple[list[ConfigProperty], dict[str, str]]) -> str:
    props, comments = items
    lines = [f"# Configuration properties — {APP_LABEL} (as-is)", ""]
    lines.append(
    ...
    overrides = _collect_profile_overrides(props)
    if overrides:
...
def collect_ports() -> list[PortIface]:
    out: list[PortIface] = []
    ...
    for port in out:
        port.impls = grep_files(rf"\bimplements\b[^;{{]*\b{re.escape(port.name)}\b", SRC_MAIN_JAVA)
        # also accept class signatures spanning multiple lines: use a permissive match
        if not port.impls:
            port.impls = grep_files(rf"implements\s+{re.escape(port.name)}\b", SRC_MAIN_JAVA)
    return out


def render_ports(ports: list[PortIface]) -> str:
    lines = [f"# Ports ↔ Adapters — {APP_LABEL} (as-is)", ""]
    lines.append(
...
def collect_jte_templates() -> list[JteTemplate]:
    jte_root = GEST_ROOT / "src" / "main" / "jte"
    ...
    for f in sorted(jte_root.rglob("*.jte")):
        ...
        out.append(JteTemplate(
            name=name, file_rel=str(f.relative_to(REPO_ROOT)),
            params=params, includes=includes,
        ))
    return out


def render_templates(templates: list[JteTemplate]) -> str:
    lines = [f"# JTE templates — {APP_LABEL} (as-is)", ""]
    lines.append(
...
def parse_jacoco_csv() -> dict[tuple[str, str], CoverageRow]:
    """Return { (package, class_simple): row } from the JaCoCo CSV. Empty if CSV absent."""
    out: dict[tuple[str, str], CoverageRow] = {}
    if not JACOCO_CSV.is_file():
        return out
    import csv as _csv
    with JACOCO_CSV.open(encoding="utf-8") as fh:
        reader = _csv.DictReader(fh)
        for row in reader:
            pkg = row["PACKAGE"]
            cls = row["CLASS"]
            # JaCoCo reports per-class incl. nested ($), JTE precompiled, etc.
            if GEST_PACKAGE_ROOT and not pkg.startswith(GEST_PACKAGE_ROOT):
                continue
...
def collect_all_main_files() -> list[tuple[str, str, str]]:
    """Return [(package, class_simple, file_rel)] for every .java under src/main/java."""
    out: list[tuple[str, str, str]] = []
    for jf in sorted(SRC_MAIN_JAVA.rglob("*.java")):
        rel = jf.relative_to(SRC_MAIN_JAVA)
        pkg = ".".join(rel.parts[:-1])
        cls = jf.stem
        out.append((pkg, cls, str(jf.relative_to(REPO_ROOT))))
    return out


def render_coverage() -> str:
    cov = parse_jacoco_csv()
    files = collect_all_main_files()
    ...
    lines = [f"# Coverage — {APP_LABEL} (as-is)", ""]
    if not cov:
        lines.append(
        ...
        mtime = _dt.datetime.fromtimestamp(JACOCO_CSV.stat().st_mtime).strftime("%Y-%m-%d %H:%M")
        lines.append(
    ...
    for r in sorted(rows, key=lambda x: x.file_rel):
        ...
        lines.append(f"| `{r.file_rel.removeprefix(f'{APP_LABEL}/src/main/java/')}` | {l or '—'} | {line_cell if l else '—'} | {branch_cell} | {meth_cell} |")
    return "\n".join(lines).rstrip() + "\n"
...
def git_blame_one(file: Path, lineno: int) -> tuple[str, int]:
...
def collect_todos() -> list[TodoItem]:
    out: list[TodoItem] = []
    for root in TODO_SCAN_ROOTS:
        if not root.is_dir():
            continue
        for f in sorted(root.rglob("*")):
            ...
            for i, line in enumerate(txt.splitlines(), start=1):
                ...
                if m:
                    ...
                    author, age = git_blame_one(f, i)
                    out.append(TodoItem(
                        kind=kind, file_rel=str(f.relative_to(REPO_ROOT)), line=i,
                ...
                if line.strip() == "@Deprecated" or line.strip().startswith("@Deprecated("):
                    author, age = git_blame_one(f, i)
                    out.append(TodoItem(
                        kind="@Deprecated", file_rel=str(f.relative_to(REPO_ROOT)), line=i,
...
def render_todos(items: list[TodoItem]) -> str:
    lines = [f"# Tech debt inventory — {APP_LABEL} (as-is)", ""]
    lines.append(
...
def parse_adrs() -> list[AdrEntry]:
    ...
    for f in sorted(ADR_DIR.glob("*.md")):
        ...
        out.append(AdrEntry(
            id=adr_id, file_rel=str(f.relative_to(REPO_ROOT)),
...
def detect_adr_candidates() -> list[tuple[str, str, str]]:
    """
    ...
    """
    out: list[tuple[str, str, str]] = []
    for f in sorted(SRC_MAIN_JAVA.rglob("*.java")):
        txt = f.read_text(encoding="utf-8", errors="replace")
    ...
    # Invariant tests
    for f in sorted((GEST_ROOT / "src" / "test" / "java").rglob("*InvariantTest.java")):
        out.append(("invariant test", str(f.relative_to(REPO_ROOT)),
                    "*InvariantTest — invariants ARE decisions; consider documenting in an ADR"))
    for f in sorted((GEST_ROOT / "src" / "test" / "java").rglob("*InvariantIT.java")):
        out.append(("invariant integration test", str(f.relative_to(REPO_ROOT)),
                    "*InvariantIT — see above"))
    # contracts
    if CONTRACTS_DIR.is_dir():
        for f in sorted(CONTRACTS_DIR.rglob("*.groovy")):
            out.append(("HTTP boundary contract", str(f.relative_to(REPO_ROOT)),
                        "Spring Cloud Contract: the boundary IS a decision (was the rule from legacy or to-be?)"))
    return out


def render_adr_index(adrs: list[AdrEntry], candidates: list[tuple[str, str, str]]) -> str:
    lines = [f"# ADR index — {APP_LABEL} (as-is)", ""]
    lines.append(
    ...
    if adrs:
        ...
        for a in adrs:
            ...
            lines.append(f"| `{a.id}` | {a.status} | {date} | [{a.title}]({a.file_rel.removeprefix(f'{APP_LABEL}/')}) | {sup} |")
        lines.append("")
    else:
...
def parse_pom_deps() -> list[Dep]:
    pom = GEST_ROOT / "pom.xml"
    if not pom.is_file():
        return []
    ...
    if parent is not None:
        ...
        deps.append(Dep("maven (parent)", g, n, v, "parent", str(pom.relative_to(REPO_ROOT))))
    for d in root.findall(".//dependencies/dependency"):
        ...
        deps.append(Dep("maven", g, n, v, s, str(pom.relative_to(REPO_ROOT))))
    return deps


def parse_npm_deps(pkg_json: Path) -> list[Dep]:
    if not pkg_json.is_file():
        return []
    import json as _json
    data = _json.loads(pkg_json.read_text(encoding="utf-8"))
    out: list[Dep] = []
    src = str(pkg_json.relative_to(REPO_ROOT))
...
def parse_python_deps() -> list[Dep]:
    ...
    for line in req.read_text(encoding="utf-8").splitlines():
        ...
        out.append(Dep("python", "", m.group(1), (m.group(2) or "").strip() or "unpinned",
                       "scripts", str(req.relative_to(REPO_ROOT))))
    return out


def collect_dependencies() -> list[Dep]:
    return (
        parse_pom_deps()
        + parse_npm_deps(GEST_ROOT / "frontend" / "package.json")
        + parse_npm_deps(GEST_ROOT / "e2e" / "package.json")
        + parse_python_deps()
    )


def render_dependencies(deps: list[Dep]) -> str:
    lines = [f"# Dependencies — {APP_LABEL} (as-is)", ""]
    lines.append(
...
def render_manifest() -> str:
    lines = [f"# MANIFEST — {APP_LABEL} auto-generated docs", ""]
    lines.append(
    ...
    lines.append("")
    existing = {p.name for p in GENERATED_DIR.glob("*.md")}
    seen = set()
...
def write_or_check(path: Path, content: str, check: bool) -> bool:
    ...
    print(f"wrote {_display_path(path)} ({len(content.splitlines())} lines)")
    return True
...
def main(argv: list[str]) -> int:
    ...
    route_constants = collect_route_constants()
    contracts = load_contracts()
    endpoints = collect_endpoints(route_constants, contracts)
    events = collect_events()
    projections = collect_projections()
    modules = collect_modules()
    migrations = parse_flyway_files()
    config = collect_config_properties()
    ports = collect_ports()
    templates = collect_jte_templates()

    targets = [
        (GENERATED_DIR / "http-endpoints.md", render_endpoints(endpoints)),
        (GENERATED_DIR / "events.md", render_events(events)),
        (GENERATED_DIR / "projections.md", render_projections(projections)),
        (GENERATED_DIR / "modules.md", render_modules(modules)),
        (GENERATED_DIR / "schema.md", render_schema(migrations)),
        (GENERATED_DIR / "config.md", render_config(config)),
        (GENERATED_DIR / "ports.md", render_ports(ports)),
        (GENERATED_DIR / "templates.md", render_templates(templates)),
        (GENERATED_DIR / "coverage.md", render_coverage()),
        (GENERATED_DIR / "todo.md", render_todos(collect_todos())),
        (GENERATED_DIR / "adr-index.md", render_adr_index(parse_adrs(), detect_adr_candidates())),
        (GENERATED_DIR / "dependencies.md", render_dependencies(collect_dependencies())),
    ]
    ...
    targets.append((GENERATED_DIR / "MANIFEST.md", ""))
    ok = True
    ...
    if not write_or_check(manifest_path, render_manifest(), check):
        ok = False
